...
class RedfinNYCRentalSpider(scrapy.Spider):
    ...
    def __init__(self, rpm: float = 5, jitter: float = 0.25,
                 start_index: int = 0, max_pages: int = 300, **kwargs):
        super().__init__(**kwargs)
        self.rpm = float(rpm)               # ~5 requests per minute
        self.jitter = float(jitter)         # ±25% randomization
    ...
    custom_settings = {
        "BIGQUERY_TABLE": "NYC_listings_rentals",
    ...
    async def parse_json(self, response):
        ...
        for home in homes:
            yield {
                ...
                "address": f"{home.get('streetLine', {}).get('value', '')}, "
                           f"{home.get('city')}, {home.get('state')} {home.get('zip')}",
